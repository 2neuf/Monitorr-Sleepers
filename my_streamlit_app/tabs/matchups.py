...
def render_matchups_tab(group_a, group_b, excluded_leagues_input, threshold_group_a, season_year="2026"):
    ...
    if not matchups_for_week:
        ...
        # Multiselect pré-sélectionnant la première rencontre par défaut
        ...
        if not sel_labels:
            st.info("Veuillez sélectionner au moins une rencontre.")
        else:
            # Récupération de l'ensemble des équipes des matchs sélectionnés
            selected_teams = set()
            ...
            if key_players_in_match.empty:
                st.info(f"Aucun joueur du Groupe A ne participe aux rencontres sélectionnées.")
            else:
